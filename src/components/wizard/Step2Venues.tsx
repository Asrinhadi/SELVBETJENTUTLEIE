...
export function Step2Venues({ needs, selectedVenueId, onSelect }: Step2Props) {
  ...
  const results = useMemo(() => rankVenues(needs), [needs])

  const suitable = results.filter((r) => r.verdict !== "ikke_egnet")
  const unsuitable = results.filter((r) => r.verdict === "ikke_egnet")
      ...
      <Card>
        ...
        <CardContent className="gap-3">
          <p className="text-base text-muted-foreground">
            Vi har sammenlignet behovene dine med {results.length} lokaler. Vurderingen er
            regelbasert, og du kan åpne forklaringen for hvert lokale.
          </p>
